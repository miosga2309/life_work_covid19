...
# visuals
byday <- aggregate(coro["SATLIFE"], by=coro["date"], FUN=mean,na.rm=TRUE)
byday$month <- format(byday$date, format = "%m")
byday$day <- format(byday$date, format = "%d")

h13_trend <- ggplot(byday, aes(x=date, y=SATLIFE)) +
  geom_point() +
  geom_smooth(method="loess",se=TRUE) +
  theme_classic()
h13_season_month <- ggplot(byday, aes(x = day, y = SATLIFE, group = month)) +
